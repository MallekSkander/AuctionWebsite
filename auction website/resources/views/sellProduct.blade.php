...
@section('body')
<br>
    <form style="font-size:20px;"  class="form-horizontal sell-form col-md-11 col-md-offset-1" role="form" method="POST" action="{{ url('sell') }}" enctype="multipart/form-data">
        {{ csrf_field() }}
        ...
        <div class="form-group{{ $errors->has('buyoutPrice') ? ' has-error' : '' }}">
            <label for="productName" class="col-md-3 control-label">buyoutPrice</label>

            <div class="col-md-8">
                <input id="buyoutPrice" type="text" class="form-control" name="buyoutPrice" value="{{ old('buyoutPrice') }}" placeholder="buyoutPrice..." required autofocus>

                @if ($errors->has('buyoutPrice'))
                    <span class="help-block">
                        <strong>{{ $errors->first('buyoutPrice') }}</strong>
                    </span>
                @endif
            </div>
        </div>
        <div class="form-group{{ $errors->has('minimalPrice') ? ' has-error' : '' }}">
            ...
            <div class="col-md-8">
                <input id="minimalPrice" type="number" class="form-control" name="minimalPrice" value="{{ old('minimalPrice') }}" placeholder="Minimal price (in .TND)..." required>
